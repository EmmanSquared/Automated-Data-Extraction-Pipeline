...
if __name__ == '__main__':
    ...
    for x in range(0,15,2):
        ...
        response = chat(
            ...
            messages=[{'role':'assistant', 'content':'you are a data entry helper, parsing images prices and turning them to csv code to copy and paste'},
            {'role': 'user', 'content': prompt, 'images': image_to_pass }],
        )

        try:
            with open (os.path.join(config.home,"Downloads",config.provinces[x],f"{config.provinces[x]}.csv"),"w") as csv_file:
